...
function getChatRoom(req, res){
    pool.getConnection((error, db)=>{
        ...
        db.query(`SELECT seller FROM product WHERE id = ${req.query.productId}`, (error, results)=>{
            ...
            db.query(`SELECT chatRoom.id, product.seller FROM chatRoom JOIN product ON chatRoom.pid = product.id WHERE chatRoom.buyer = '${req.decoded.userId}' AND chatRoom.pid = ${req.query.productId}`, (error, results)=>{           
                if(error){
                    db.release();
                    return res.status(400).json({error: error});
                }
        ...
        db.query(`INSERT INTO chatRoom(buyer, pid, date) VALUES('${req.decoded.userId}', ${req.body.productId}, '${req.body.date}')`, (error, results)=>{
            ...
            db.query(`INSERT INTO chat(message, date, status, rid, sender) VALUES('${req.body.message}', '${req.body.date}', 0, ${roomId}, '${req.decoded.userId}')`, (error)=>{                
                ...
                db.query(`SELECT seller FROM product WHERE id = ${req.body.productId}`, (error, results)=>{
                    ...
                    const receiverArray = [...(userToSocketMap.get(req.decoded.userId) ? userToSocketMap.get(req.decoded.userId) : []), ...(userToSocketMap.get(results[0].seller) ? userToSocketMap.get(results[0].seller) : [])];
